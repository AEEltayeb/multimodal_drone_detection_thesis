...
Run:  python -B gui/pyside_app.py
"""
...
from flet_app.settings_dialog import PATH_FILE_KEYS, PATH_DIR_KEYS
...
class TalosWindow(QMainWindow):
    ...
    def _on_play_pause(self):
        ...
        if self.current_mode == "Single Model" and self.single_model_type == "ir":
            ...
            # Single-model runs only the rgb_model branch, so route the IR MLP
            # verifier through the rgb MLP slot — this lets you test the IR
            # model's MLP on a single IR video. When the IR MLP is off, disable
            # the MLP entirely: the RGB MLP was trained on RGB-YOLO features and
            # is out-of-distribution on the IR detector, so it must not run here.
            if run_settings.get("use_ir_mlp_verifier"):
                run_settings["use_mlp_verifier"] = True
                run_settings["mlp_verifier_weights"] = run_settings.get("ir_mlp_verifier_weights", "")
                run_settings["mlp_threshold"] = float(run_settings.get("ir_mlp_threshold", 0.25))
                run_settings["mlp_filter_mode"] = run_settings.get("ir_mlp_filter_mode", "per_frame")
                run_settings["mlp_alert_gate_conf"] = float(run_settings.get("ir_mlp_alert_gate_conf", 0.4))
            else:
                run_settings["use_mlp_verifier"] = False
        ...
        # Tell the engine which single-model modality is loaded so the MLP
        # verifier UI labels it correctly (single-IR routes through the rgb slot).
        self.engine.single_modality = (
            self.single_model_type if self.current_mode == "Single Model" else "rgb")
    ...
    def _on_frame(self, jpeg_bytes, stats):
        ...
        # Confuser filter status card. When the MLP V5 verifier is active it
        # replaces the RGB patch verifier, so the badge + status reflect MLP.
        v = stats.get("verifier") or {}
        suppressed = stats.get("confuser_suppressed", False)
        mlp = stats.get("mlp") or {}
        ir_mlp = stats.get("ir_mlp") or {}

        def _mlp_detail(m, tag):
            """One/two-line P(drone) summary for an MLP verifier diag dict.
            The modality tag is shown once on the header line; the P(drone)
            line is an indented continuation of the same verifier."""
            n_in = m.get("n_in", 0); n_kept = m.get("n_kept", 0)
            thr = m.get("threshold", 0.25)
            d = f"{tag}: kept {n_kept}/{n_in}  thr={thr:.2f}"
            mx, mn = m.get("max_p"), m.get("min_p")
            if mx is not None: d += f"\n      P(drone) max={mx:.2f} min={mn:.2f}"
            return d

        if stats.get("mlp_active") or stats.get("ir_mlp_active"):
            self.gate_badge.setText("MLP V5")
            self.gate_badge.setStyleSheet(
                "background:#0a3d62; color:#54d2ff; font-size:9px; font-weight:700; "
                "padding:2px 6px; border-radius:4px;")
            self._last_suppressed = False
            # Headline reflects whichever branch(es) vetoed this frame.
            rgb_vet = mlp.get("n_vetoed", 0) if stats.get("mlp_active") else 0
            ir_vet = ir_mlp.get("n_vetoed", 0) if stats.get("ir_mlp_active") else 0
            total_vet = rgb_vet + ir_vet
            if total_vet > 0:
                self.filter_status.setText(f"MLP VETOED {total_vet}")
                self.filter_status.setStyleSheet("font-size:13px; font-weight:700; color:#ff4444;")
            else:
                self.filter_status.setText("MLP PASS")
                self.filter_status.setStyleSheet("font-size:13px; font-weight:700; color:#54d2ff;")
            detail_lines = []
            if stats.get("mlp_active"):
                detail_lines.append(_mlp_detail(mlp, stats.get("mlp_label", "RGB")))
            if stats.get("ir_mlp_active"):
                detail_lines.append(_mlp_detail(ir_mlp, "IR"))
            self.filter_detail.setText("\n".join(detail_lines))
        elif suppressed:
            self.gate_badge.setText("ALERT GATE")
            self.gate_badge.setStyleSheet(
                "background:#333; color:#aaa; font-size:9px; font-weight:700; "
                "padding:2px 6px; border-radius:4px;")
    ...
    # -- SETTINGS ----------------------------------------
    def _browse_path_field(self, key, edit):
        """Open a file/folder picker for a settings path field and write the
        chosen path back into its line edit."""
        start = edit.text().strip()
        start_dir = os.path.dirname(start) if start and os.path.dirname(start) else start
        if key in PATH_DIR_KEYS:
            p = QFileDialog.getExistingDirectory(self, f"Select {LABELS.get(key, key)}", start_dir)
        else:
            ...
            p, _ = QFileDialog.getOpenFileName(
                self, f"Select {LABELS.get(key, key)}", start_dir, filt)
        if p:
            edit.setText(p)
        ...
        for sec, keys in SECTIONS:
            ...
            for k in keys:
                if k in CHOICE_KEYS:
                    ...
                    refs[k] = cb
                    form.addRow(LABELS.get(k, k), cb)
                elif k in PATH_FILE_KEYS or k in PATH_DIR_KEYS:
                    le = QLineEdit(str(self.settings.get(k, DEFAULTS.get(k, ""))))
                    refs[k] = le
                    browse = QPushButton("...")
                    browse.setFixedWidth(36)
                    browse.clicked.connect(
                        lambda _checked=False, key=k, edit=le: self._browse_path_field(key, edit))
                    row = QHBoxLayout()
                    row.setContentsMargins(0, 0, 0, 0)
                    row.addWidget(le, 1)
                    row.addWidget(browse)
                    row_w = QWidget(); row_w.setLayout(row)
                    form.addRow(LABELS.get(k, k), row_w)
                else:
                    le = QLineEdit(str(self.settings.get(k, DEFAULTS.get(k, ""))))
                    refs[k] = le
