...
# --- Paths ---
# Set via apply_runtime_config() from the API before starting detection
MODEL_WEIGHTS = ""
VIDEO_PATH = ""
...
# (computed at runtime in main() since VIDEO_PATH is set dynamically)
...
REQUESTED_SEEK_ABS = -1 # Absolute seek target frame (-1 = no seek)
...
ALERT_AVG_CONF_THRESHOLD = 0.0  # 0.0 = disabled; >0 = avg confidence over alert-hit frames must exceed this
...
# --- Simple Mode (bypass all advanced logic, basic YOLO like app.py) ---
SIMPLE_MODE = False
...
alert_conf_window = deque(maxlen=int(ALERT_WINDOW_FRAMES))  # confidence per slot (0.0 for misses)
inference_rows = []
...
def overlay_text(img, lines, x=None, y=None, line_h=None):
    h, w = img.shape[:2]
    scale = max(0.35, h / 900.0 * 0.55)
    thickness_bg = max(2, int(h / 900.0 * 3))
    thickness_fg = max(1, int(h / 900.0 * 1))
    if x is None: x = int(w * 0.012)
    if line_h is None: line_h = int(h * 0.028)
    if y is None: y = line_h + int(h * 0.01)
    for i, t in enumerate(lines):
        # Truncate if text is wider than frame
        max_chars = int(w / (scale * 12))
        t = t[:max_chars]
        yy = y + i * line_h
        cv2.putText(img, t, (x, yy), cv2.FONT_HERSHEY_SIMPLEX, scale, (0, 0, 0), thickness_bg, cv2.LINE_AA)
        cv2.putText(img, t, (x, yy), cv2.FONT_HERSHEY_SIMPLEX, scale, (255, 255, 255), thickness_fg, cv2.LINE_AA)


def overlay_text_big(img, lines, x=None, y=None, line_h=None, scale=None):
    """Big-font overlay for TOPLEFT_LOG_MODE='windows_big'. Scales to frame size."""
    h, w = img.shape[:2]
    if scale is None: scale = max(0.6, h / 900.0 * 1.35)
    thickness_bg = max(3, int(h / 900.0 * 6))
    thickness_fg = max(1, int(h / 900.0 * 2))
    if x is None: x = int(w * 0.02)
    if line_h is None: line_h = int(h * 0.055)
    if y is None: y = line_h + int(h * 0.015)
    for i, t in enumerate(lines):
        max_chars = int(w / (scale * 14))
        t = t[:max_chars]
        yy = y + i * line_h
        cv2.putText(img, t, (x, yy), cv2.FONT_HERSHEY_SIMPLEX, float(scale), (0, 0, 0), thickness_bg, cv2.LINE_AA)
        cv2.putText(img, t, (x, yy), cv2.FONT_HERSHEY_SIMPLEX, float(scale), (255, 255, 255), thickness_fg, cv2.LINE_AA)
...
def main(headless=False, frame_callback=None, stop_event=None, pause_event=None):
    global REQUESTED_SEEK_REL, REQUESTED_SEEK_ABS
    ...
    if SIMPLE_MODE:
        stride = 1  # Simple mode: infer every frame
    else:
    ...
    playback_speed = float(PLAYBACK_SPEED) if float(PLAYBACK_SPEED) > 0 else 1.0  # initial; re-read each frame
    ...
    while True:
        ...
        # Frame skipping for playback speed > 1x
        # At 2x, skip 1 frame; at 4x, skip 3 frames (grab without decode)
        ...
        # Headless Control: Absolute Seek
        if REQUESTED_SEEK_ABS >= 0:
            try:
                new_pos = int(REQUESTED_SEEK_ABS)
                if total_frames > 0:
                    new_pos = min(new_pos, total_frames - 1)
                ...
                REQUESTED_SEEK_ABS = -1

        is_infer = (frame_id % stride == 0)
        vis = frame.copy()

        # --- SIMPLE MODE: basic YOLO predict (like app.py) ---
        if SIMPLE_MODE:
            infer_t0 = time.perf_counter()
            num_infer_frames += 1
            res = model.predict(frame, conf=float(DETECT_CONF), max_det=int(MAX_FULLFRAME_DETECTIONS),
                               verbose=False, imgsz=int(BASE_IMGSZ), save=False)[0]
            s_boxes, s_confs = [], []
            if res.boxes is not None and len(res.boxes) > 0:
                for i in range(len(res.boxes)):
                    if int(res.boxes.cls[i]) == DRONE_CLASS_ID:
                        x1, y1, x2, y2 = res.boxes.xyxy[i].cpu().numpy()
                        s_boxes.append((float(x1), float(y1), float(x2), float(y2)))
                        s_confs.append(float(res.boxes.conf[i]))
            total_yolo_calls += 1
            infer_t1 = time.perf_counter()
            total_infer_time_s += (infer_t1 - infer_t0)
            last_infer_final_boxes = s_boxes
            last_infer_final_confs = s_confs
            last_infer_final_sources = ["full"] * len(s_boxes)
            last_infer_final_main_confs = list(s_confs)
            last_infer_final_post_confs = [None] * len(s_boxes)
            is_infer = False  # skip complex inference block below
        ...
        if is_infer:
            ...
            if alert_cooldown_left <= 0:
                ...
                # Track confidence per slot for avg-conf gate
                alert_conf_window.append(float(rep_main_conf) if (alert_hit and rep_main_conf is not None) else 0.0)
            ...
            # Average confidence gate: require avg conf over hit frames >= threshold
            alert_avg_conf_val = 0.0
            if alert_active_pre and float(ALERT_AVG_CONF_THRESHOLD) > 0:
                hit_confs = [c for c, h in zip(alert_conf_window, win_alert) if h]
                alert_avg_conf_val = (sum(hit_confs) / len(hit_confs)) if hit_confs else 0.0
                if alert_avg_conf_val < float(ALERT_AVG_CONF_THRESHOLD):
                    alert_active_pre = False  # avg conf too low, demote
        ...
        if SIMPLE_MODE:
            # Simple app.py-style drawing: green/yellow/red boxes + "drone 0.XX" labels
            for b, c in zip(last_infer_final_boxes, last_infer_final_confs):
                x1, y1, x2, y2 = map(int, b)
                color = (0, 255, 0) if c > 0.6 else (0, 255, 255) if c > 0.4 else (0, 0, 255)
            ...
            if TOPLEFT_LOG_MODE != "off":
                if TOPLEFT_LOG_MODE == "windows_big":
                    # Compute avg conf over alert-hit frames for display
                    _hit_confs_disp = [c for c, h in zip(alert_conf_window, win_alert) if h]
                    _avg_conf_disp = (sum(_hit_confs_disp) / len(_hit_confs_disp)) if _hit_confs_disp else 0.0
                    _avg_conf_str = f"  avgConf={_avg_conf_disp:.2f}" if float(ALERT_AVG_CONF_THRESHOLD) > 0 else ""
                    lines = [
                        ...
                        f"ALERT   hits {sum(win_alert)}/{len(win_alert)}  need {ALERT_REQUIRE_HITS}/{ALERT_WINDOW_FRAMES}{_avg_conf_str}  events={count_alert_events}",
                    ]
                ...
                _hit_confs_full = [c for c, h in zip(alert_conf_window, win_alert) if h]
                _avg_conf_full = (sum(_hit_confs_full) / len(_hit_confs_full)) if _hit_confs_full else 0.0
                _avg_conf_full_str = f" avgConf={_avg_conf_full:.2f}" if float(ALERT_AVG_CONF_THRESHOLD) > 0 else ""
                ...
                lines = [
                    ...
                    f"Windows: Warn hits {sum(win_warning)}/{len(win_warning)} (need {WARNING_REQUIRE_HITS}/{WARNING_WINDOW_FRAMES})  |  Alert hits {sum(win_alert)}/{len(win_alert)} (need {ALERT_REQUIRE_HITS}/{ALERT_WINDOW_FRAMES}){_avg_conf_full_str}",
        ...
        if headless and frame_callback:
            ...
            # Re-read playback speed each frame for dynamic control
            playback_speed = float(PLAYBACK_SPEED) if float(PLAYBACK_SPEED) > 0 else 1.0

            # Pace headless playback to source FPS (prevents video running too fast)
