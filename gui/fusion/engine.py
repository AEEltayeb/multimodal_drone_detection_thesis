...
try:
    ...
    MLPVerifier = None
    DetectInputHook = None
    _MLP_IMPORT_ERROR = _exc
else:
    _MLP_IMPORT_ERROR = None
...
class FusionEngine:
    ...
    def __init__(
        ...
        use_mlp_verifier: bool = False,
        mlp_verifier_weights: Optional[str] = None,
        mlp_threshold: float = 0.25,
        mlp_filter_mode: str = "per_frame",
        mlp_alert_gate_conf: float = 0.4,
        # IR-branch MLP V5 verifier (mirror of the RGB one, hooked on ir_model)
        use_ir_mlp_verifier: bool = False,
        ir_mlp_verifier_weights: Optional[str] = None,
        ir_mlp_threshold: float = 0.25,
        ir_mlp_filter_mode: str = "per_frame",
        ...
        # features for each RGB detection and scores P(drone) at ~50x lower
        # cost. The IR patch verifier (real thermal) is unaffected. See
        # docs/analysis/mlp_v5_report.md.
        self.use_mlp_verifier = bool(use_mlp_verifier)
        self.mlp_threshold = float(mlp_threshold)
        self.mlp_filter_mode = mlp_filter_mode if mlp_filter_mode in (
            "per_frame", "alert_gate") else "per_frame"
        self.mlp_alert_gate_conf = float(mlp_alert_gate_conf)
        self.mlp_verifier = None
        self.mlp_hook = None
        self._mlp_hook_handle = None
        if self.use_mlp_verifier:
            if MLPVerifier is None:
                print(f"[Fusion] MLP verifier unavailable: {_MLP_IMPORT_ERROR}")
                self.use_mlp_verifier = False
            elif not (mlp_verifier_weights and Path(mlp_verifier_weights).exists()):
                print(f"[Fusion] MLP verifier requested but weights not found "
                      f"({mlp_verifier_weights}); disabled.")
                self.use_mlp_verifier = False
            else:
                if isinstance(device, int):
                    import torch
                    mlp_device = f"cuda:{device}" if torch.cuda.is_available() else "cpu"
                else:
                    mlp_device = str(device)
                print(f"[Fusion] Loading MLP V5 verifier: {mlp_verifier_weights}")
                self.mlp_verifier = MLPVerifier(mlp_verifier_weights, mlp_device)
                self.mlp_hook = DetectInputHook()
                self._mlp_hook_handle = self.mlp_hook.register(self.rgb_model)
                print(f"[Fusion] MLP V5 ON (thr={self.mlp_threshold}, "
                      f"schema={self.mlp_verifier.feature_schema}, "
                      f"cv_f1={self.mlp_verifier.cv_f1:.4f}). "
                      f"RGB patch verifier disabled — MLP takes its place.")

        # IR-branch MLP V5 verifier (optional). Same feature-distillation idea
        # as the RGB one, but the hook is registered on `ir_model` and it scores
        # the IR detections. Used in Paired / Grayscale fusion to verify the IR
        # branch (and, via the single-model path, to test the IR MLP on its own).
        self.use_ir_mlp_verifier = bool(use_ir_mlp_verifier)
        self.ir_mlp_threshold = float(ir_mlp_threshold)
        self.ir_mlp_filter_mode = ir_mlp_filter_mode if ir_mlp_filter_mode in (
            "per_frame", "alert_gate") else "per_frame"
        self.ir_mlp_alert_gate_conf = float(ir_mlp_alert_gate_conf)
        self.ir_mlp_verifier = None
        self.ir_mlp_hook = None
        self._ir_mlp_hook_handle = None
        if self.use_ir_mlp_verifier:
            if MLPVerifier is None:
                print(f"[Fusion] IR MLP verifier unavailable: {_MLP_IMPORT_ERROR}")
                self.use_ir_mlp_verifier = False
            elif not (ir_mlp_verifier_weights and Path(ir_mlp_verifier_weights).exists()):
                print(f"[Fusion] IR MLP verifier requested but weights not found "
                      f"({ir_mlp_verifier_weights}); disabled.")
                self.use_ir_mlp_verifier = False
            else:
                if isinstance(device, int):
                    import torch
                    ...
                    mlp_device = str(device)
                print(f"[Fusion] Loading IR MLP V5 verifier: {ir_mlp_verifier_weights}")
                self.ir_mlp_verifier = MLPVerifier(ir_mlp_verifier_weights, mlp_device)
                self.ir_mlp_hook = DetectInputHook()
                self._ir_mlp_hook_handle = self.ir_mlp_hook.register(self.ir_model)
                print(f"[Fusion] IR MLP V5 ON (thr={self.ir_mlp_threshold}, "
                      f"schema={self.ir_mlp_verifier.feature_schema}, "
                      f"cv_f1={self.ir_mlp_verifier.cv_f1:.4f}).")

        # Patch verifiers (optional). When the MLP verifier is active it owns
        # the RGB branch, so we skip loading the RGB patch verifier entirely.
        ...
        load_rgb_patch = not self.use_mlp_verifier
        if use_patch_verifier and PatchVerifier is not None:
            ...
            if load_rgb_patch and rgb_patch_weights and Path(rgb_patch_weights).exists():
    ...
    def mlp_score_dets(self, dets, img_shape):
        """Score detections with the MLP V5 verifier using the hook's current
        feature maps. Call IMMEDIATELY after the rgb_model forward that
        produced `dets` (before the next forward overwrites the hook).

        dets: iterable of [x1, y1, x2, y2, conf]. img_shape: (H, W).
        Returns (n,) P(drone), or None if the MLP verifier is disabled.
        """
        if self.mlp_verifier is None or self.mlp_hook is None:
            return None
        return self.mlp_verifier.score_dets(self.mlp_hook, dets, img_shape)

    def ir_mlp_score_dets(self, dets, img_shape):
        """Score IR detections with the IR MLP V5 verifier using the IR hook's
        current feature maps. Call IMMEDIATELY after the ir_model forward that
        produced `dets`. Returns (n,) P(drone), or None if disabled."""
        if self.ir_mlp_verifier is None or self.ir_mlp_hook is None:
            return None
        return self.ir_mlp_verifier.score_dets(self.ir_mlp_hook, dets, img_shape)
