...
Stages:
  ...
  2. Fine-tune YOLO (outputs RGB model/<name>/weights/best.pt)
  3. Eval old vs new on dataset_rgb test split + selcom val split
  4. Print comparison table + save comparison.json

Usage:
    python "RGB model/finetune_selcom.py"
    python "RGB model/finetune_selcom.py" --name Yolo26n_selcom_mixed_ft1 --ratio 0.20
    python "RGB model/finetune_selcom.py" --skip-stage --skip-train  # eval-only
    python "RGB model/finetune_selcom.py" --no-mix                   # pure selcom (no replay)
"""
