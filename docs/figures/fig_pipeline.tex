...
\begin{document}
    \begin{tikzpicture}[>=Stealth, font=\small,
      ...
      \node[box] (clf)  at (5.9,0)   {XGBoost trust\\classifier\\(4-way, \texttt{robust8})};
      \node[ver] (ver)  at (9.3,0)   {confuser filter\\(MLP; \textbf{per-frame};\\ROI-feature reuse)};
      \node[box] (temp) at (12.6,0)  {Temporal\\smoother\\($N$-of-$M$)};
      \node[box, fill=red!10] (alert) at (15.6,0) {ALERT};
      \node[old] (patch) at (9.3,-2.5) {patch filter\\(predecessor; alert-gated)};
      \draw[->] (rgb)--(ry);   \draw[->] (ir)--(iy);
      \draw[->] (ry)--(clf);   \draw[->] (iy)--(clf);
      \draw[->] (clf)--(ver);  \draw[->] (ver)--(temp); \draw[->] (temp)--(alert);
