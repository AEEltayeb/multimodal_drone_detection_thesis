...
The remainder of this thesis is organised as follows. Chapter~\ref{ch:literature} reviews the relevant literature on real-time object detection, drone-detection datasets and the confuser problem, thermal-infrared imaging, multi-modal fusion, hard-negative mining, detection scoring, and cross-modal transfer. Chapter~\ref{ch:architecture} presents the system architecture: the five-stage cascade, the asymmetric-recoverability design principle, the trust-aware fusion model, the alert-gate cascade, and the temporal smoother. Chapter~\ref{ch:components} covers the individual components: the three RGB detector variants, the SelCom CCTV fine-tune, the IR detector with its six-revision HITL co-evolution trace, the trust classifier and its three variants, the patch verifier, and the label-reviewer GUI. Chapter~\ref{ch:methodology} describes the methodology: datasets and their composition, the IoP scoring rule, the scoring-rule audit, the Svanstr\"om usage / leakage audit, and the reproducibility infrastructure. Chapter~\ref{ch:experiments} reports the experimental results: cumulative confuser suppression, classifier comparison, patch threshold sweep, patch catch-rate audit, the Roboflow OOD audit, the real-video six-mode diagnostic, the grayscale cross-modal transfer finding, the full cascade on real video, SelCom CCTV evaluation, and threats to validity. Chapter~\ref{ch:conclusion} answers the four research questions, summarises the production stack, and lists future-work directions. Appendix~\ref{app:datasets} provides the consolidated per-clip / per-split dataset metadata; Appendix~\ref{app:glossary} a glossary of abbreviations and shorthand tokens used throughout.
...
The pipeline processes paired RGB and thermal-IR streams through a four-stage cascade. Each stage's empirical contribution is measured independently in Chapter~\ref{ch:experiments}; this chapter presents the architecture and the design choices behind it.
...
% ======================================================================
% CHAPTER 4 — COMPONENT DESIGN AND HITL
% ======================================================================
\chapter{Component Design and HITL}
\label{ch:components}
...
The IR detector uses the same YOLOv26n architecture as the RGB detector, finetuned on iteratively curated thermal data (\S\ref{sec:hitl_ir}). Its training corpus includes bird, airplane, and helicopter thermal frames as hard negatives, fed into the dataset across HITL revisions whenever the previous IR detector hallucinated on a confuser scene (\S\ref{sec:hitl_ir}). The production weights (\texttt{runs/corrective\_finetune/finetune\_v3b/weights/best.pt}, hereafter \texttt{v3b}) are a 2-epoch corrective fine-tune on top of the \texttt{Final} checkpoint and address specific deployment-time failure modes.

\subsection{IR Detector Evolution: Precision and Recall on a Fixed Test Split}
\label{sec:ir_evolution}

The IR detector was developed across six numbered revisions (V2 through V6) plus a \texttt{Final} retrain and a \texttt{v3b} corrective fine-tune. Earlier discussions of this work reported a per-version best-epoch mAP50 number from each model's training run, which is not directly comparable across versions because the val split changed with the dataset. To produce comparable numbers, every version was re-evaluated on the same fixed \texttt{IR\_dset\_final} test split at \texttt{imgsz=640} (\textsc{Ledger}~\S4.1; script: \texttt{eval/eval\_ir\_versions.py}; output: \texttt{eval/results/ir\_version\_comparison/ir\_comparison\_test\_640\_2026-05-16T20-04-39.\{json,csv\}}).
...
The IR-detector trajectory the table records can be read in four passes.
...
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}
        \textbf{[PLACEHOLDER: Fig 4.2 --- V5 regression qualitative]} 3--4 frame panel showing scenes where V4 was correct and V5 fires on newly-ingested but unlabelled drones (scored as FP). Same scenes after corpus cleanup (V6) where the label is corrected and the model fires correctly. Concretises the dataset-state $\leftrightarrow$ model-state coupling described in §\ref{sec:hitl_ir}.
    \vspace{2cm}}}
    \caption{V5 regression case study: same scenes scored as FP under V5 (label missing) and as TP under V6 (label corrected). The model state is unchanged across the V5--V6 transition in these specific frames; the dataset state is what moved.}
...
The IR detector was not trained once on a static dataset. It was co-developed with the dataset across the six revisions of Table~\ref{tab:ir_evolution}. Each revision follows the same loop:
...
This chapter presents the empirical results. \S\ref{sec:cumulative} reports cumulative confuser suppression across cascade stages. \S\ref{sec:classifier_compare} compares the three trust-classifier variants. \S\ref{sec:patch_threshold} and \S\ref{sec:patch_audit} cover the patch verifier's operating point and per-bucket catch rate. \S\ref{sec:roboflow} reports the Roboflow OOD audit. \S\ref{sec:realvideo} reports the six-mode real-video diagnostic and Pareto analysis; \S\ref{sec:grayscale} examines the unexpected cross-modal transfer. \S\ref{sec:realvideo_cascade} runs the full cascade on real video. \S\ref{sec:selcom} reports the SelCom CCTV evaluation. \S\ref{sec:threats} and \S\ref{sec:limits} list threats to validity and limitations.
...
\section{Patch Catch-Rate Audit}
