...
\subsection[Confuser Hard-Negative Corpus]{Confuser Hard-Negative Corpus (\texttt{rgb\_confusers\allowbreak\_merged})}
...
The production router always routes; it never abstains. A reject-capable variant, which can also veto a whole frame, is kept as an ablation and analysed in Section~\ref{sec:classifier_results}. The router's eight features were screened by the leakage statistic of Section~\ref{sec:model_mri}, which drops any feature that discriminates \emph{which scene} a frame came from rather than \emph{whether the detections are trustworthy}, so the router cannot pass its evaluation by memorising surface identity. The strongest hand-built comparison, a 32-feature classifier, leaks exactly those scene statistics, so its in-domain lead is read as an upper bound rather than a deployable target.
...
\begin{enumerate}
    \item \textbf{Baseline.} Trained on the composite drone corpus. On Svanstr\"om@1280 the bare detector, no downstream stage applied, reaches $P=0.940, R=0.961$ on drones, with a 94.4\% bird-frame fire rate.
...
The deployed PySide GUI exposes a \emph{grayscale-RGB} operating mode (\texttt{\_infer\_\allowbreak grayscale}). When no thermal camera is available, the GUI converts the RGB stream to grayscale via \texttt{cvtColor(\allowbreak BGR2GRAY)}, replicates to three channels, and feeds the result to the IR detector at \texttt{imgsz=640}. This was built as a conservative hardware fallback; its measured behaviour (competitive with the dedicated RGB detector on bird-cluttered scenes, a few points behind it overall) turned out to be one of the thesis's findings (Section~\ref{sec:grayscale}).
...
\subsubsection{Statistical Feature Selection for the Router}
...
\subsection{The Confuser Filter}
