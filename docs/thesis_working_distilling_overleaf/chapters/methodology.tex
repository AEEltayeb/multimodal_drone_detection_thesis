...
\begin{table}[htbp]
    ...
    \begin{tabular}{lll}
        ...
        IR filter (thermal \texttt{mlp\_aligned\_thermalonly} / & In-distribution (Svanstr\"om IR crops in & Clean (Anti-UAV val only; val$\cap$test $= 0$) \\
        \quad gray \texttt{mlp\_aligned\_gray\_balanced}) & thermal-head training) & \\
        \bottomrule
    \end{tabular}
\end{table}
...
% [source: mri/train_aligned.py GROUPS (thermal head mlp_aligned_thermalonly: svanstrom IR crops in training; Anti-UAV val split, val/test segment overlap = 0; gray head mlp_aligned_gray_balanced same overlap verdict); knowledge/models.csv mlp_v5_balanced_v4 (rgb_dataset/rgb_confusers train+val, pure selcom, bird.v1i train)]
...
This thesis follows a \emph{statistics-before-training} discipline for its downstream learned components: design decisions are settled by measuring what a detector's features encode before any training run. The discipline applies to exactly three models: the feature-reuse RGB filter \texttt{mlp\_v5\_v4} (Section~\ref{sec:distill_verifier}), the cross-modal IR filters (thermal-native \texttt{mlp\_aligned\_thermalonly} and grayscale-aligned \texttt{mlp\_aligned\_gray\_balanced}) via IR feature separability and grayscale$\leftrightarrow$thermal alignment (Section~\ref{sec:ir_xmodal_verifier}), and the trust router's feature selection that produced \texttt{robust6}/\texttt{robust8} (Section~\ref{sec:feature_selection}). The detectors themselves were trained conventionally, and the hand-engineered \texttt{sa32} comparison classifier predates the discipline. The feature-space evidence is produced by the \emph{Model MRI} tool (\texttt{python -m mri}; modules \texttt{mri/\{extract,classifier,holdout,modality\}.py}).
...
The MRI is not only a diagnostic; its output \emph{is} the filter's training pipeline. The same 517-D per-detection embeddings it extracts for analysis become the training matrix for a small multi-layer perceptron (MLP), trained automatically with 5-fold cross-validation when the statistics support it (\texttt{mri/classifier.py}); held-out evaluation (\texttt{mri/holdout.py}) gates the result on a corpus excluded from training. The production RGB filter (\texttt{mlp\_v5\_v4}) and the cross-modal IR filters (\texttt{mlp\_aligned\_thermalonly} and \texttt{mlp\_aligned\_gray\_balanced}) are all products of this path: measure first, train only when the measurement says the signal is there, and verify on a held-out gate.
...
\begin{enumerate}
    ...
    \item \textbf{Confuser filter}: in production, the distilled \texttt{mlp\_v5\_v4} RGB feature-space filter (and its two IR heads) (Section~\ref{sec:distill_verifier}). Because it reuses the detectors' already-computed ROI features rather than re-processing crops, it is cheap enough to run \emph{per frame} on every detection. It supersedes a 4-class MobileNetV3 patch filter which, being a separate CNN over crop pixels, was expensive enough to require alert-gating (Section~\ref{sec:patch_verifier_arch}).
\end{enumerate}
...
\paragraph{Two-filter fusion (trust-first).} With both the RGB (\texttt{mlp\_v5\_v4}) and IR filters running per-frame, fusion is \emph{trust-first}: the trust classifier (Section~\ref{sec:trust_fusion}) routes, then the trusted modality's confuser filter screens the surviving detections. \texttt{reject\_both} drops the frame; \texttt{trust\_rgb} keeps it only if the RGB filter passes; \texttt{trust\_ir} keeps it only if the IR filter passes; \texttt{trust\_both} is resolved per-modality (recall-first: the detection survives if either trusted filter passes). The IR filter is \emph{two heads}: a thermal-native CBAM net (\texttt{mlp\_aligned\_thermalonly}, $P(\text{drone})\geq 0.05$) on the thermal-deploy path, and a grayscale-aligned net (\texttt{mlp\_aligned\_gray\_balanced}, $P(\text{drone})\geq 0.25$) on the grayscale-fallback path (Section~\ref{sec:ir_xmodal_verifier}). The composition order (classifier$\to$filter vs filter$\to$classifier) is itself ablated in Section~\ref{sec:pipeline_ablation}.
...
\begin{table}[htbp]
    \centering
    \caption{Svanstr\"om drone recall by RGB variant and inference size (one harness: stride-7 sample, $n=4{,}102$, IoP@0.5, conf $0.25$). Doubling \texttt{imgsz} lifts both variants by a similar increment ($+28$/$+25$~pp); the absolute level is set by the training stance, and the confuser-suppressing \texttt{retrained\_v2} is unusable on Svanstr\"om at either size.}
    \label{tab:resolution}
    \begin{tabular}{lcc}
        \toprule
        RGB variant & \texttt{imgsz}=640 & \texttt{imgsz}=1280 \\
        \midrule
        baseline               & $0.684$ & $0.964$ \\
        \texttt{retrained\_v2} & $0.070$ & $0.323$ \\
        \bottomrule
    \end{tabular}
\end{table}
% [source: eval/results/svan_resolution_sweep.json (baseline@640 0.6838 / @1280 0.9641; retrained_v2@640 0.0699 / @1280 0.3234; stride 7, n=4102, conf 0.25, IoP@0.5); run=eval/svan_resolution_sweep.py]
...
\emph{Second, the grayscale$\rightarrow$thermal feature gap is a removable affine offset.} The detector represents a given class almost identically in real thermal and in grayscale-RGB (top-discriminative-neuron overlap Jaccard $0.71$--$0.88$; mean-activation correlation $0.93$--$0.99$; drone-centroid cosine distance $0.012$); the only systematic difference is a per-feature mean/scale offset. Removing it with a per-modality z-score lifts grayscale$\rightarrow$thermal confuser-transfer from chance to near the within-thermal ceiling, far above what full-covariance CORAL recovers (Table~\ref{tab:gray_thermal_auroc}), confirming the gap is diagonal.
% [source: ledger=gray-thermal-alignable; run=mri/modality_align.py; cache=mri/results/ir_v3b_report; config=ir_v3b]

\begin{table}[htbp]
    \centering
    \caption{Grayscale$\rightarrow$thermal confuser-transfer (drone-vs-confuser AUROC). A per-modality z-score recovers nearly the full within-thermal ceiling; full-covariance CORAL recovers only part, so the modality gap is a diagonal per-feature offset rather than a representational change. Figure~\ref{fig:ir_gray_align} plots these as bars.}
    \label{tab:gray_thermal_auroc}
    \begin{tabular}{lc}
        \toprule
        alignment & transfer AUROC \\
        \midrule
        raw (no alignment) & $0.500$ (chance) \\
        CORAL (full covariance) & $0.707$ \\
        per-modality z-score & $0.919$ \\
        within-thermal ceiling ($5$-fold CV) & $0.974$ \\
        \bottomrule
    \end{tabular}
\end{table}
% [source: mri/docs/ir_grayscale_verifier_report.md sec.4; ledger=gray-thermal-alignable (raw 0.500 / CORAL 0.707 / z-score 0.919 / ceiling 0.974)]

Production therefore ships \emph{two} $517$-D heads. The \emph{thermal-native} head (\texttt{mlp\_aligned\_thermalonly}, a CBAM-trained net) is trained on $8{,}112$ thermal drones and $2{,}045$ \texttt{IR\_confusers/train} confusers (Svanstr\"om-IR, Anti-UAV-val-IR, \texttt{ir\_dset\_final}-train thermal drones plus the CBAM \texttt{train} drones, GT-aware); the \emph{grayscale-aligned} head (\texttt{mlp\_aligned\_gray\_balanced}) is built from grayscale-harvested confusers per-modality z-aligned into thermal feature space, the affine-offset result above licensing the cross-modal transfer. The earlier single-net design (one network with two per-modality input scalers) was superseded because thermal-native training recovers held-out CBAM drone recall ($0.717 \to 0.967$) that the shared net could not. The grayscale path remains recall-limited (grayscale Svanstr\"om recall drops $0.55 \to {\sim}0.08$ at its $0.25$ threshold), so grayscale detection runs largely unfiltered: the grayscale head is a confuser-suppression tool, not a recall-safe gate.
% [source: docs/analysis/2026-06-18_filter_provenance_train_heldout.md §2.1-2.4 (two-head IR: thermal-native mlp_aligned_thermalonly recovers CBAM recall 0.717->0.967 the shared net could not; grayscale-aligned mlp_aligned_gray_balanced @0.25, gray_svan recall ~0.08, confuser-suppression-only); knowledge/models.csv mlp_aligned_thermalonly, mlp_aligned_gray_balanced]
The thermal-native head holds drone recall on held-out thermal surfaces (antiuav\_ir $0.937$, ir\_video $0.971$, svanstrom\_ir $0.966$); the only cost is \texttt{ir\_dset\_final} $0.965 \to 0.928$ ($-3.7$~pp), on genuinely airplane-like drones that no threshold separates. It cuts CBAM confuser false positives from $48$ to $6$ (held-out CBAM \texttt{valid} split). The thermal head is trained on \texttt{IR\_confusers/train} confusers plus Svanstr\"om-IR / Anti-UAV-val / \texttt{ir\_dset\_final}-train thermal drones, and is evaluated on the held-out CBAM \texttt{valid} split and the \texttt{IR\_confusers} val/test re-mine (TEST split, disjoint from training); the held-out results are in Section~\ref{sec:verifier_results}. All numbers are reported on each dataset's TEST split; Svanstr\"om-IR is the in-sample exception (no held-out split exists; the drone recall there is held flat $0.966$ but is in-sample).
...
% [source: ledger=filter-score-as-classifier-feature; run=classifier/verifier_as_feature_stats.py; cache=docs/analysis/2026-06-02_forward_selection_study.md]
...
Feature reuse makes the filter $46$--$72\times$ faster per detection than the patch CNN ($1.3$--$2.1$~ms vs $59$--$112$~ms; $1$--$4\%$ pipeline overhead), which is what makes \emph{per-frame} verification affordable and removes the alert-gate coupling of the predecessor design. On the shipped corpus the MLP reaches 5-fold CV $F1$ of $0.9857\pm0.0004$, rejecting $97\%$ of confuser detections while retaining $98.9\%$ of true drones. Its IR counterparts are \emph{two heads} (Section~\ref{sec:ir_xmodal_verifier}): the thermal-native \texttt{mlp\_aligned\_thermalonly} and the grayscale-aligned \texttt{mlp\_aligned\_gray\_balanced}.
