...
\begin{table}[htbp]
    ...
    \begin{tabular}{lll}
        ...
        RGB filter (\texttt{mlp\_v5}) & Clean (SelCom CCTV features only) & Clean \\
        IR filter (aligned)          & In-distribution (Svanstr\"om IR crops) & Clean (val split only; val$\cap$test $= 0$) \\
        \bottomrule
    \end{tabular}
\end{table}
...
% [source: mri/train_aligned.py GROUPS (aligned filter: svanstrom IR crops; Anti-UAV val split, val/test segment overlap = 0); knowledge/models.csv mlp_v5 (pure selcom corpus)]
...
\begin{table}[htbp]
    \centering
    \caption{Held-out clean split versus the full-surface headline. Clean cells carry 95\% bootstrap CIs; the production pipeline is router~$\to$~filter at the canonical configuration.}
    ...
    \begin{tabular}{llll}
        ...
        \quad \textbf{production pipeline} & \textbf{0.935} [0.928--0.941] & 0.949 & $-1.4$~pp \\
        \midrule
        ...
        \quad \textbf{production pipeline} & \textbf{0.986} [0.986--0.987] & 0.984 & $+0.2$~pp \\
        \bottomrule
    \end{tabular}
\end{table}
...
Three conclusions follow. First, \emph{Anti-UAV shows no inflation at all}: every arm scores at or slightly above its full-surface headline on the clean segments, so the overlapped segments were, if anything, the harder ones; the no-harm conclusion and the production decisions anchored on Anti-UAV hold on $57{,}542$ held-out frames. Second, \emph{the Svanstr\"om IR detector was inflated, boundedly}: its solo $F1$ drops $7.3$~pp on clean sequences, but the RGB detector, which has no Svanstr\"om anywhere in its training, drops $3.5$~pp on the same sequences, so part of the gap is sequence difficulty and $7.3$~pp is an upper bound on the leakage component. Third, \emph{the cascade conclusion is robust}: the production pipeline moves only $-1.4$~pp, the smallest change of any arm, and still lifts paired $F1$ from $0.684$ to $0.935$ on sequences nothing in the detectors ever saw. The relative ordering of all router variants is unchanged on clean data, so no production decision is affected. One residual overlap remains disclosed rather than removed: the trust router's training rows were mined from both surfaces, so cascade evaluation runs on router-training sequences even in the clean split; the router's own train/test protocol is sequence-stratified, and a stricter subset excluding router-training sequences (measured on the cached evaluation sample) showed the pipeline gain persists.
...
The production checkpoint \texttt{mlp\_v5\_ir\_aligned} therefore combines thermal drone detections (for recall safety; a $\sim$30k drone-diversity re-mine made the filter recall-safe on held-out thermal) with grayscale-harvested confusers, per-modality z-aligned into a single $517$-D filter. It is \emph{one network with two per-modality input scalers}: a thermal scaler and a grayscale scaler share the same weights and differ only in the $(\mu,\sigma)$ standardisation folded into the input (a per-feature z-score shift; the measurement that revealed this offset is the affine-offset result above and Section~\ref{sec:mri_findings}), so a single trained filter serves both the thermal-deploy and grayscale-fallback paths. This is genuinely \emph{one} shipped network, not two: a separately trained grayscale-only filter was the rejected alternative, because on grayscale Svanstr\"om it over-vetoes drones (recall $0.55 \to 0.16$) where the single aligned network stays recall-safe ($0.55 \to 0.51$).
% [source: ledger=mini-offline-verifier-matrix-1k (gray\_svan drone R: aligned 0.548->0.505 vs dedicated grayscale net 0.548->0.164); DECISIONS 2026-06-09 single IR verifier; models.csv mlp_v5_gray (dedicated grayscale net, superseded/unused)]
The alignment costs nothing on thermal: versus the bare detector, the aligned filter is recall-neutral on held-out thermal surfaces ($\Delta R = -0.007$ on the IR test split, $0.000$ on IR video, $0.000$ on Anti-UAV) while cutting CBAM confuser false positives from $48$ to $15$. The CBAM probe (Section~\ref{sec:ds_ir_confusers}) is held out of its training; the held-out results are in Section~\ref{sec:verifier_results}.
% [source: knowledge/evals.csv ir_aligned_cbam_heldout (canonical dual-save re-run: CBAM F1 0.699->0.846, FP 48->15, R 0.967->0.917; dR -0.007/0.000/0.000 ir_dset/ir_video/antiuav); models.csv mlp_v5_ir (un-aligned predecessor: net-negative recall, superseded)]
