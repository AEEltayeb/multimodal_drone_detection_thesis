...
Single-run point estimates are never compared at sub-CI granularity; the production-stack choices are justified on cost and OOD behavior where in-domain differences are within noise (Section~\ref{sec:pipeline_ablation}).
...
\paragraph{IR OOD recall.} The IR detector's recall on independent, verified thermal drone footage outside its dataset families is unmeasured (the available community thermal sets could not be quality-verified and are not used); this is the evaluation gap the RGB parallel channel hedges against.
