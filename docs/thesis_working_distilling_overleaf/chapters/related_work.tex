...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{4.0cm}lll}
        ...
        \textbf{This thesis} & RGB + IR (decision-level) & Trust classifier + per-frame distilled (\texttt{mlp\_v5}) confuser filter & Learned per-frame modality choice; cascade designed for fail-open \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lllccc}
        ...
        \midrule
        \multirow{2}{*}{Svanstr\"om (visible video)} & Svanstr\"om 2021 (avg.\ over distance bins) & IoU@0.5, det.\ thr.\ 0.5, YOLOv2 @416 & ND & ND & 0.785 \\
                                                   & \textbf{This thesis (baseline RGB, S1)} & IoP@0.5, @1280 & 0.940 & 0.961 & 0.950 \\
        \midrule
        \multirow{2}{*}{Svanstr\"om (thermal video)}  & Svanstr\"om 2021 (avg.\ over distance bins) & IoU@0.5, det.\ thr.\ 0.5, YOLOv2 @256 & ND & ND & 0.760 \\
        ...
        \midrule
        Confuser corpus (no GT, RGB)               & \emph{No published baseline} & ND & ND & ND & ND \\
                                                   & \textbf{This thesis (confuser filter, @640)} & fire-rate & ND & ND & 1.1\% fire \\
        \midrule
        Thermal confuser corpus (no GT, IR)        & \emph{No published baseline} & ND & ND & ND & ND \\
                                                   & \textbf{This thesis (best composition, @640)} & fire-rate & ND & ND & 29.4\% $\to$ 17.9\% fire \\
        \bottomrule
    \end{tabular}
\end{table}
% [source: thesis_eval/results/tier1_results.json (ir_confusers C: bare 0.2943, clf->filt[robust6] 0.1792)]
...
Two claims survive these caveats. First, this thesis's per-modality numbers \emph{exceed} the published Svanstr\"om averages ($0.950$ vs $0.785$ visible, $0.961$ vs $0.760$ IR); the caveats above carry the qualification, since part of that gap is attributable to detector generation (YOLOv26n vs YOLOv2), inference resolution ($1280$/$640$ vs $416$/$256$), and matcher leniency (IoP vs IoU) rather than to a like-for-like superiority. At minimum, the detectors are competitive baselines rather than weak strawmen. Second, no published system reports a separately-evaluated OOD confuser fire rate at the granularity of Table~\ref{tab:ablation_confusers}, because no published system explicitly separates a confuser-rejection stage from drone detection. The suppression numbers of Section~\ref{sec:pipeline_confusers} ($30.4\% \to 4.9\%$ at the router, $\to 1.1\%$ at the per-frame filter, at the canonical configuration) therefore cannot be directly benchmarked. The contribution is the architectural separation that makes such numbers measurable.
