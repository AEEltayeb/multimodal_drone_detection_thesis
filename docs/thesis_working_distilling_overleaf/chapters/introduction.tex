...
No single sensing modality meets both cheaply, and the Counter-UAS (C-UAS) surveys explain why~\cite{shi2018counteruas,taha2019drone,samaras2019deep}. Radar struggles with the small radar cross-section of quadcopter-class drones. Radio-frequency detection needs an active control link, which autonomous and fiber-optic-guided drones (fielded at scale since 2024) do not emit~\cite{csis2025dronewar}. Acoustic arrays degrade under wind, traffic, and aircraft noise. Visual detection with deep neural networks is passive, inexpensive, and context-rich, and is the modality this thesis adopts.

Modern single-stage detectors are genuinely strong at this task. The production RGB detector built here reaches $P=0.989$, $R=0.982$ on the Anti-UAV benchmark~\cite{jiang2021antiuav}, with only 41 false positives across 4{,}000 frames, and it hallucinates on just $2.8\%$ of its in-distribution test corpus.\footnote{Anti-UAV frames appear in the detector's training corpus, so this is an in-distribution sanity floor rather than a generalisation claim (Section~\ref{sec:lit_drone}); evaluated at \texttt{imgsz=640}, $n=4{,}000$.} On ordinary footage (sky, buildings, traffic, vegetation), the false-alarm problem is, for these detectors, already solved at the detector.
...
The false-alarm problem is not generic. On most footage the detector never fires falsely at all; the failures concentrate in scenes with the three aerial \emph{confuser} classes: birds, fixed-wing aircraft, and helicopters. On a 2{,}633-image out-of-distribution confuser corpus assembled for this thesis, the same detector fires on $30.4\%$ of frames, and on bird-only Svanstr\"om footage that rises to $94\%$ (the per-category breakdown is in Table~\ref{tab:rgb_comparison}; example hallucinations in Figure~\ref{fig:confuser_fp_examples}).
...
The thesis is organised around three research questions; the answers are interpreted in Chapter~\ref{ch:conclusion}.
...
This thesis makes four contributions and reports one finding.
...
\begin{enumerate}
    ...
    \item \textbf{A set of curated data artifacts.} The product of the manual review behind the system: a $129{,}130$-frame reviewed thermal corpus; a $27{,}024$-frame RGB confuser corpus with sequence-disjoint splits; the $28{,}710$-frame paired Svanstr\"om conversion to YOLO format; and a $19$-clip real-video benchmark ($2{,}609$ frames, $1{,}234$ annotated drone instances) that is fully out-of-distribution for every component. Licences preclude redistributing the source corpora, so these are claimed as reviewed artifacts; the real-video benchmark's labels are publishable (Appendix~\ref{app:datasets}).
...
One finding emerged that was not designed. Run zero-shot on grayscale-converted RGB, the thermal-trained detector still detects drones when the visible channel fails: it lands within a few points of the dedicated RGB detector on the same frames ($F1$ $0.580$ vs $0.607$ on Svanstr\"om) and ties it on the hardest bird-cluttered clip ($0.837$ vs $0.840$). It is a usable fallback for when no thermal camera is present (Section~\ref{sec:grayscale}).
...
Counter-UAS sensing is dual-use technology. The same passive visual-detection capability that flags a hostile drone over critical infrastructure could, in a different deployment, contribute to mass aerial surveillance or armed-response targeting. This thesis develops the detection component only: it neither integrates with effectors (jammers, kinetic intercept, take-down systems) nor classifies operator behaviour. The training corpora contain no personally identifying information, no identifiable third-party imagery beyond what is already public in the cited datasets and clips, and no acoustic or RF data that could identify an operator. The deployment scope assumed throughout is the protection of fixed sites (perimeters, airport approaches, industrial facilities), where a detected drone is reported to a human operator who decides the response. Because every result is reproducible from the published caches (Appendix~\ref{app:provenance}), any future adaptation of the system remains auditable.

\section{Thesis Outline}
\label{sec:outline}

Chapter~\ref{ch:literature} reviews the literature: real-time object detection, drone detection and the confuser problem, thermal imaging, multi-modal fusion, cross-modal transfer and cascaded rejection, and the two methodological lineages this work builds on (data-centric annotation and feature-space probing). Chapter~\ref{ch:methodology} presents the system and the experimental method: the architecture and each component, the datasets and their roles, the Model MRI and the human-in-the-loop engine, and the evaluation protocol. Chapter~\ref{ch:hitl} reports the results: the full-pipeline ablation that answers RQ1--RQ3, the per-component evidence (including the thermal detector's six-revision evolution), the Model MRI findings, and the grayscale cross-modal finding. Chapter~\ref{ch:conclusion} interprets the answers to the research questions, states the production stack with its honest carve-outs, discusses threats and limitations, and lists future work. The appendices detail the datasets (Appendix~\ref{app:datasets}), the models (Appendix~\ref{app:models}), and the glossary (Appendix~\ref{app:glossary}).
