"""
build_selcom_mixed_ft1.py — Stage a mixed dataset for selcom fine-tuning.

Combines:
  - ALL selcom training images (from RGB_TestDrone_dense_0_6440_selcom_footage_ft1)
  - A random sample of general RGB training images (G:/drone/dataset/dataset)

to prevent catastrophic forgetting when fine-tuning on the small selcom set.

Default ratio: 80% general / 20% selcom  (--ratio 0.20 means selcom is 20%)

Output: G:/drone/_finetune_selcom_mixed_ft1/
  images/{train,val}/
  labels/{train,val}/
  data.yaml
  manifest.csv
...
SELCOM_ROOT   = Path(r"G:/drone/RGB_TestDrone_dense_0_6440_selcom_footage_ft1"
...
DST_ROOT = Path(r"G:/drone/_finetune_selcom_mixed_ft1")

IMG_EXTS  = {".jpg", ".jpeg", ".png", ".bmp"}
SEED      = 0
VAL_FRAC  = 0.15   # val comes from selcom only


def _selcom_items():
    """Yield (img_path, lbl_path_or_None, is_positive) for all selcom images."""
    ...
    for img in imgs:
        ...
        out.append((img, lbl if has else None, has))
    return out


def _general_sample(n: int, rng: random.Random):
    """Return n randomly sampled (img_path, lbl_path) pairs from the general train set."""
...
def build(selcom_ratio: float = 0.20, clean: bool = False) -> Path:
    ...
    # ── General: compute how many to sample ──────────────────────────────────
    n_selcom_train = len(selcom_train)
    # selcom_ratio = n_selcom / (n_selcom + n_general)  →  n_general = n_selcom * (1 - r) / r
    ...
    def copy_pair(img_src, lbl_src_or_none, split, source_tag, is_positive):
        # Prefix filename with source tag to avoid collisions
        ...
        if lbl_src_or_none is not None and lbl_src_or_none.exists() and lbl_src_or_none.stat().st_size > 0:
    ...
    # selcom val (only selcom — keeps val interpretable for selcom distribution)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--ratio", type=float, default=0.20,
                    help="Fraction of training images that come from selcom (default 0.20)")
