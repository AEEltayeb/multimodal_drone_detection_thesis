...
CHECKS += [
    ...
    ("SWING trust-aware pipeline F1", 0.9485, T["svanstrom"]["B_pipeline"]["clf->filt[robust8]"]["f1"]),
    ("SWING identical precision", 0.9388, N["svanstrom"]["M_modality_ab"]["routed[robust8] +filt"]["precision"]),
    ("SWING trust-aware precision", 0.9388, T["svanstrom"]["B_pipeline"]["clf->filt[robust8]"]["precision"]),
]

# CBAM aligned-filter FP is stated in two places (methodology §3.8.8 prose + empirical
# tab:ir_aligned) and has NO replay JSON (canonical = evals.csv ir_aligned_cbam_heldout, 48->15).
# A 2026-06-13 sweep caught the prose drifting to 13 while the table was correct, undetected
# because only the table was audited. Pin both restatements to the canonical value and to each other.
import re
_CBAM_CANON_FP = 15
...
_m_meth = re.search(r"CBAM confuser false positives from \$48\$ to \$(\d+)\$", _meth)
_m_emp = re.search(r"0\.786/0\.917/0\.846\}\$?\s*\(\\textbf\{(\d+)\}\)", _emp)
CHECKS += [
...
CHECKS += [
    ...
    ("SWEEP selcom filt@0.05", 0.692, sweep_row("selcom_val", 0.05)["filt"]["f1"]),
]
...
CHECKS += [
    ...
    ("CLEAN svan pipeline", 0.9348, C["svanstrom_clean"]["B_pipeline"]["clf->filt[robust8]"]["f1"]),
    ...
    ("CLEAN auv pipeline", 0.9862, C["antiuav_clean"]["B_pipeline"]["clf->filt[robust8]"]["f1"]),
]
...
CHECKS += [
    ...
    ("DUT clf->filt[robust8] P", 0.898, round(DB["clf->filt[robust8]"]["precision"], 3)),
    ("DUT filt->clf[robust8] P", 0.900, round(DB["filt->clf[robust8]"]["precision"], 3)),
]
...
CHECKS += [
    ("NR svan composed F1",  0.930, round(TN["svanstrom"]["B_pipeline"][f"clf->filt[{_NR}]"]["f1"], 3)),
    ("NR svan composed R",   0.960, round(TN["svanstrom"]["B_pipeline"][f"clf->filt[{_NR}]"]["recall"], 3)),
    ("NR svan composed P",   0.902, round(TN["svanstrom"]["B_pipeline"][f"clf->filt[{_NR}]"]["precision"], 3)),
    ("NR svan filt->clf F1", 0.944, round(TN["svanstrom"]["B_pipeline"][f"filt->clf[{_NR}]"]["f1"], 3)),
    ("NR antiuav composed F1", 0.984, round(TN["antiuav"]["B_pipeline"][f"clf->filt[{_NR}]"]["f1"], 3)),
    ("NR dut composed F1",   0.792, round(DBN[f"clf->filt[{_NR}]"]["f1"], 3)),
    ("NR dut composed R",    0.728, round(DBN[f"clf->filt[{_NR}]"]["recall"], 3)),
    ("NR dut filt->clf F1",  0.837, round(DBN[f"filt->clf[{_NR}]"]["f1"], 3)),
    ("NR rgb_conf fire",     0.0106, round(TN["rgb_confuser"]["C_confuser"][f"clf->filt[{_NR}]"]["fire_rate"], 4)),
    ("NR ir_conf fire",      0.237, round(TN["ir_confusers"]["C_confuser"][f"clf->filt[{_NR}]"]["fire_rate"], 3)),
    # shipped composition filt->clf on confusers: identical fire (no-reject router never vetoes a confuser frame)
    ("NR rgb_conf fire filt->clf", 0.0106, round(TN["rgb_confuser"]["C_confuser"][f"filt->clf[{_NR}]"]["fire_rate"], 4)),
    ("NR ir_conf fire filt->clf",  0.237, round(TN["ir_confusers"]["C_confuser"][f"filt->clf[{_NR}]"]["fire_rate"], 3)),
    ("NR rgb_test clf F1",   0.926, round(TN["rgb_dataset_test"]["S4_verifier"][f"clf[{_NR}]"]["f1"], 3)),
    ("NR rgb_test composed F1", 0.809, round(TN["rgb_dataset_test"]["S4_verifier"][f"clf->filt[{_NR}]"]["f1"], 3)),
    ...
    ("NR video composed F1", 0.687, round(VN["video_drone"][f"clf->filt[{_NR}]"]["window"][2], 3)),
    ("NR video_conf fire",   0.2252, round(VN["video_confuser"][f"clf->filt[{_NR}]"]["window_fire"], 4)),
    ("NR AB svan routed+filt F1", 0.909, round(NN["svanstrom"]["M_modality_ab"]["routed[robust8-nr] +filt"]["f1"], 3)),
    ("NR AB antiuav routed+filt F1", 0.974, round(NN["antiuav"]["M_modality_ab"]["routed[robust8-nr] +filt"]["f1"], 3)),
    ...
    ("dut filt_mlp_rgb F1",    0.726, round(DB["filt_mlp_rgb"]["f1"], 3)),
    ("dut filt_mlp_ir F1",     0.725, round(DB["filt_mlp_ir"]["f1"], 3)),
]
...
CHECKS += [
    ...
    ("FIG gray recall@0.25", 0.467, round(FS["grayscale aligned"]["shipped"][0], 3)),
    ("FIG gray recall@0.05", 0.618, round(FS["grayscale aligned"]["t0.05"][0], 3)),
    ("FIG gray fire@0.25",   0.008, round(FS["grayscale aligned"]["shipped"][1], 3)),
    ("FIG gray fire@0.05",   0.036, round(FS["grayscale aligned"]["t0.05"][1], 3)),
]
