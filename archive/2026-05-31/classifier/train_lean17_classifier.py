"""train_lean17_classifier.py - 17-feature XGBoost.
Lean-13 base + rgb/ir pos_x + rgb/ir dist_to_center.
"""
...
def main():
    ...
    ap.add_argument("--csv", default="classifier/fusion_models/lean17/fusion_dataset_lean17.csv")
    ap.add_argument("--output-dir", default="classifier/fusion_models/lean17")
    args = ap.parse_args()
    ...
    print(f"Loaded {len(df):,} rows from {csv_p.name}")
    ...
    print(f"Train: {len(Xtr):,}  Test: {len(Xte):,}  Features: {len(FEATURE_COLS)}")
    ...
    json.dump({
        "tag": "fusion_lean17", "n_features": len(FEATURE_COLS),
